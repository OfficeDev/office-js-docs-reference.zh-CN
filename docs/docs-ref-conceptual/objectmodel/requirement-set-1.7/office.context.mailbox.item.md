...
#### <a name="from-emailaddressdetailsjavascriptapioutlook17officeemailaddressdetailsfromjavascriptapioutlook17officefrom"></a>从：[EmailAddressDetails](/javascript/api/outlook_1_7/office.emailaddressdetails)|[从](/javascript/api/outlook_1_7/office.from)
...
*   [EmailAddressDetails](/javascript/api/outlook_1_7/office.emailaddressdetails) | [从](/javascript/api/outlook_1_7/office.from)
...
`organizer`属性返回一个代表会议组织者的[EmailAddressDetails](/javascript/api/outlook_1_7/office.emailaddressdetails)对象。

##### <a name="compose-mode"></a>撰写模式

`organizer`属性返回一个[管理器](/javascript/api/outlook_1_7/office.organizer)对象，提供要获取组织者值的方法。
...
#### <a name="nullable-recurrence-recurrencejavascriptapioutlook17officerecurrence"></a>(可以为 null) 的定期：[定期](/javascript/api/outlook_1_7/office.recurrence)

获取或设置约会的定期模式。 获取会议请求的定期模式。 阅读和撰写的约会项目的模式。 读取模式的会议请求项目。

`recurrence`属性返回的定期约会或会议请求的[定期](/javascript/api/outlook_1_7/office.recurrence)对象，如果项是一系列或一系列中的实例。 `null`将返回单个约会和会议请求的单个约会。 `undefined`将返回不是会议请求的消息。
...
####  <a name="to-arrayemailaddressdetailsjavascriptapioutlook17officeemailaddressdetailsrecipientsjavascriptapioutlook17officerecipients"></a>到： 数组。 <[EmailAddressDetails](/javascript/api/outlook_1_7/office.emailaddressdetails)>|[收件人](/javascript/api/outlook_1_7/office.recipients)
...
|名称|类型|属性|说明|
|---|---|---|---|
|`callback`|函数||方法完成后，使用单个参数 `callback`（一个 [`asyncResult`](/javascript/api/office/office.asyncresult) 对象）调用在 `AsyncResult` 参数中传递的函数。<br/><br/>自定义属性作为 `asyncResult.value` 属性中的 [`CustomProperties`](/javascript/api/outlook_1_7/office.customproperties) 对象提供。 此对象可用于获取、 设置，和从项目中删除自定义属性并将更改保存到自定义属性设置回服务器。|
